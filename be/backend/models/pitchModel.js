...
async function searchPitches({ search, location, minPrice, maxPrice, pitchType }) {
  const connection = await db.getConnection();
  try {
    let query = `SELECT * FROM pitches WHERE status = 'available'`;
    const params = [];
...
async function updatePitch(pitchId, { name, location, price_per_hour, pitch_type_id, status, avatar }) {
  ...
  await connection.execute(
    ...
    [name, location, price_per_hour, pitch_type_id, status, pitchId, avatar]
  );
  connection.release()
}
